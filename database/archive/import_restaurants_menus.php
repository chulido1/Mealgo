...
try {
    ...
    if ($menusByRestaurant === []) {
        throw new RuntimeException('Файл меню пустой или не распознан.');
    }

    $pdo = db();
    $restaurants = $pdo->query('SELECT id, name FROM restaurants ORDER BY id ASC')->fetchAll(PDO::FETCH_ASSOC);
    if ($restaurants === []) {
        throw new RuntimeException('В таблице restaurants нет записей. Сначала выполните schema.sql + seed.sql.');
    }
    ...
    $insertCategory = $pdo->prepare(
        'INSERT INTO menu_categories (restaurant_id, name, sort_order, is_active, created_at, updated_at)
         VALUES (:restaurant_id, :name, :sort_order, 1, NOW(), NOW())'
    );
    ...
    foreach ($menusByRestaurant as $restaurantName => $categories) {
        $restaurantId = (int) $restaurantMap[$restaurantName]['id'];
        $sortOrder = 10;
    ...
    echo "Готово.\n";
    echo "Ресторанов обработано: " . count($menusByRestaurant) . "\n";
    echo "Категорий создано: {$categoriesInserted}\n";
    echo "Позиции меню создано: {$itemsInserted}\n";
} catch (Throwable $e) {
...
{
    if (!is_file($filePath)) {
        throw new RuntimeException('Не найден файл: ' . $filePath);
    }
